...
namespace rpc
{
Connection::Connection(ip::tcp::socket s)
	: m_socket(std::move(s))
{
}
...
void Connection::OnReadBody(const boost::system::error_code& error)
{
	...
	if (!error)
	{
	    spdlog::get("console")->info("connection on read msg body: \" %s \"", m_readmsg.body());
...
void Connection::ReadHead()
{
	...
	spdlog::get("console")->info("connection start async read head length %u", m_readmsg.headerlen());
...
void Connection::ReadBody()
{
	...
	spdlog::get("console")->info("connection start async read body length %u", m_readmsg.bodylen());
